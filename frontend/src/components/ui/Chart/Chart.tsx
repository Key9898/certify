...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      return <div className={cn("font-bold uppercase tracking-widest text-[10px] text-base-content/40", labelClassName)}>{value}</div>
    }, [
      label,
      labelFormatter,
      payload,
      ...
      >
        ...
        <div className="grid gap-1.5 line-height-tight">
          {payload.map((item) => {
            ...
            const formattedValue = formatter ? formatter(item.value, item.name) : item.value
