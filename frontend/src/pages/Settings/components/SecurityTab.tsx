import { Shield, Mail, RefreshCw, LogOut, AlertTriangle, Trash2, KeyRound, ArrowRight, Globe } from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      >
        <div className="flex items-center gap-4 mb-10 pb-8 border-b border-base-200/60">
          ...
          <div>
            <h2 className="text-2xl font-black tracking-tight text-base-content uppercase tracking-widest text-[18px]">Security Protocol</h2>
            <p className="text-sm text-base-content/55 font-medium leading-relaxed">Guard your access points and credential integrity.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1.2fr_0.8fr] gap-10">
            ...
            <div className="p-6 bg-base-200/30 rounded border border-base-200 group hover:border-info/20 transition-all">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <p className="meta-label">Credential Standard</p>
                </div>
                <span className="brand-tag !bg-info/10 !text-info">
                  {isEmailPasswordUser ? 'Direct Hub' : 'OAuth Verified'}
                </span>
              </div>
              <div className="space-y-1">
                <p className="text-xl font-black text-base-content tracking-tight">{isEmailPasswordUser ? 'Certify Email Auth' : 'Google Identity'}</p>
                <p className="text-sm font-medium text-base-content/45">Verified account: {userEmail}</p>
              </div>
            </div>

            <div className="space-y-6">
              <div className="flex items-center gap-3">
                <h3 className="text-lg font-black tracking-tight text-base-content border-l-4 border-info pl-4">Security Actions</h3>
              </div>
              ...
              {isEmailPasswordUser ? (
                <div className="space-y-4 pt-2">
                  <div className="p-6 rounded bg-base-100 border border-base-200 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                      <p className="font-black text-base-content text-lg mb-2">Change Password</p>
                      <p className="text-sm font-medium text-base-content/50 leading-relaxed mb-6 max-w-sm">
                        To maintain direct Hub security, we send an encrypted reset orchestrator to your verified address <strong>{userEmail}</strong>.
                      </p>
                      
                      {pwResetStatus === 'sent' && (
                        <motion.div initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }} className="alert border-success/30 bg-success/10 py-3 rounded gap-3 mb-4 ring-1 ring-success/20">
                          <Mail size={16} className="text-success" />
                          <span className="text-xs font-black uppercase tracking-widest text-success">Security token dispatched to your inbox!</span>
                        </motion.div>
                      )}
                      
                      {pwResetStatus === 'error' && (
                        <div className="alert border-error/30 bg-error/10 py-3 rounded mb-4 gap-3">
                          <Shield size={16} className="text-error" />
                          <span className="text-xs font-black uppercase tracking-widest text-error">Dispatch pipeline failure. Please retry.</span>
                        </div>
                      )}
                      ...
                      <Button
                        ...
                        className="h-11 px-8 font-black uppercase tracking-widest text-[11px] rounded shadow-lg shadow-primary/20"
                      >
                        <RefreshCw size={14} className="mr-0" />
                        Init Password Reset
                      </Button>
                    </div>
                  </div>
                ...
                <div className="p-8 rounded bg-success/5 border border-success/10 group hover:border-success/20 transition-all">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h4 className="font-black text-xl text-base-content tracking-tight">External Identity Protection</h4>
                  </div>
                  <p className="text-sm font-medium text-base-content/55 leading-relaxed mb-6">
                    Your Hub credentials are encrypted and strictly managed through Google Workspace. Direct password manipulation is handled by their global security infrastructure.
                  </p>
                  <a
          ...
          <div className="space-y-8">
            <div className="p-8 bg-base-200/20 rounded border border-base-200 relative overflow-hidden h-fit">
              ...
              <p className="meta-label mb-3 ml-1">Current Portal State</p>
              <h3 className="text-xl font-black text-base-content tracking-tight mb-2">Session Integrity</h3>
              <p className="text-sm font-medium text-base-content/50 leading-relaxed mb-8">
                Instantly revoke current browser tokens and end all active connections on this local portal.
              </p>
              <Button 
                variant="ghost" 
              ...
              >
                <LogOut size={16} /> Secure Portal Sign Out
              </Button>
            </div>
            ...
            <div className="p-8 border border-error/20 bg-error/[0.03] rounded group transition-all hover:bg-error/[0.06] hover:border-error/40 shadow-sm shadow-error/5">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <h3 className="font-black text-xl text-error tracking-tight">Danger Zone</h3>
                  <p className="meta-label !text-error/40 italic">Terminal Deletion Protocol</p>
                </div>
              </div>
              ...
              {!showDeleteConfirm ? (
                <div className="space-y-6">
                  <p className="text-sm font-medium text-base-content/60 leading-relaxed">
                    Permanently purge your account, all created certificates, secure templates, and entire workspace history from our global infrastructure.
                  </p>
                  <Button
                    variant="error"
                  ...
                  >
                    <Trash2 size={16} /> Authorize Account Purge
                  </Button>
                </div>
              ) : (
                <div className="space-y-5">
                  <div className="alert border-error/40 bg-error/10 py-4 rounded gap-3 text-error">
                    ...
                    <span className="text-xs font-black uppercase tracking-widest">This Operation is Final and Irreversible</span>
                  </div>
                  <div className="space-y-3">
                    <label className="label h-6 p-0 ml-1">
                      <span className="meta-label !text-error/60">Type 'DELETE' to confirm terminal protocol</span>
                    </label>
                    <input
                      type="text"
                  ...
                  {deleteError && (
                    <p className="text-[10px] font-black uppercase tracking-widest text-error text-center px-4">{deleteError}</p>
                  )}
