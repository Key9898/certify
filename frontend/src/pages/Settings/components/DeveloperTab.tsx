      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-base-content/55 font-medium leading-relaxed">
                Secure credentials for external application integration.
              </p>
            </div>
          </div>
        ...
        {createdKey && (
          <div className="alert border-warning/30 bg-warning/5 flex-col items-start gap-4 p-6 rounded mb-8 relative border-2 border-dashed">
            <div className="flex items-center gap-2">
              ...
              <p className="meta-label !text-warning">
                Secure Token Generated — Store it now
              </p>
            </div>
            <div className="flex items-center gap-3 w-full bg-base-100 p-4 rounded border border-base-200 shadow-inner">
              <code className="font-mono text-sm leading-none text-base-content flex-1 break-all tracking-tight font-black">
                {createdKey}
              </code>
              <motion.button
              ...
              >
                {keyCopied ? (
                  ...
                  <Copy size={18} className="text-base-content/40" />
                )}
              </motion.button>
            </div>
            <p className="text-xs font-bold text-base-content/40 italic flex items-center gap-1.5">
              <ShieldAlert size={12} /> This credential is encrypted and cannot
              be retrieved after dismissal.
            </p>
            <button
              className="btn btn-xs btn-ghost text-[10px] font-black absolute top-4 right-4"
        ...
        <div className="space-y-3 mb-8">
          <p className="meta-label italic px-1 mb-4 flex items-center gap-2">
            <span className="h-1 w-1 rounded-full bg-base-content/20" /> Active
            Credentials
          </p>
          {apiKeys.length === 0 && !createdKey ? (
            <div className="p-10 border-2 border-dashed border-base-200 rounded text-center">
              <p className="text-sm font-bold text-base-content/40 italic">
              ...
              {apiKeys.map((k, index) => (
                ...
                >
                  <div className="min-w-0">
                    <p className="font-black text-sm text-base-content tracking-tight">
                      {k.name}
                    </p>
                    <p className="font-mono text-[11px] text-base-content/40 mt-1 uppercase tracking-widest">
                      {k.keyPreview}
                    </p>
                  </div>
                  <button
        ...
        {keyError && (
          <p className="text-error text-[11px] font-black uppercase tracking-widest mt-4 px-2">
            {keyError}
          </p>
        )}
      </motion.div>
      ...
      >
        <div className="flex items-start justify-between gap-4 mb-8 pb-6 border-b border-base-200/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-base-content/55 font-medium leading-relaxed">
                Real-time callbacks to your centralized processing apps.
              </p>
            </div>
          </div>
          <div className="hidden lg:flex items-center gap-2 bg-base-200/40 px-4 py-2 rounded border border-base-200">
          ...
          <div className="alert border-secondary/30 bg-secondary/5 flex-col items-start gap-4 p-6 rounded mb-8 relative border-2 border-dashed">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[11px] font-black uppercase tracking-[0.2em] text-secondary">
                Signature Secret Generated — Critical Item
              </p>
            </div>
            <div className="flex items-center gap-3 w-full bg-base-100 p-4 rounded border border-base-200 shadow-inner">
              <code className="font-mono text-sm leading-none text-base-content flex-1 break-all tracking-tight font-black">
            ...
            <p className="text-xs font-bold text-base-content/40 italic flex items-center gap-1.5">
              <ShieldAlert size={12} /> This secret validates incoming Certify
              requests. It cannot be recovered later.
            </p>
            <button
              className="btn btn-xs btn-ghost text-[10px] font-black absolute top-4 right-4"
        ...
        <div className="space-y-4 mb-10">
          ...
          {webhooks.length === 0 ? (
            <div className="p-10 border-2 border-dashed border-base-200 rounded text-center">
              <p className="text-sm font-bold text-base-content/40 italic">
                No outbound delivery routes configured.
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="min-w-0 pr-4">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <p className="font-mono text-[13px] font-black text-base-content/80 truncate tracking-tight">
                        {wh.url}
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {wh.events.map((ev) => (
                        <span
                          key={ev}
                          className="badge badge-xs h-5 px-3 py-0 border-none bg-base-200 text-base-content/50 font-black uppercase tracking-widest text-[9px]"
                        >
                          {ev.replace('certificate.', '')}
                        </span>
                      ))}
                    </div>
                  </div>
